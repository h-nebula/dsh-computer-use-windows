...
function runPs(args: string[], timeoutMs: number): Promise<PsResult> {
...
function psOut(res: PsResult): unknown {
...
function tool(
  ...
  return defineTool({
    ...
    output: { schema: OUT as never, render: renderText },
    timeoutMs: 30000,
    async execute(args, exec) {
      return execute(args, exec) as never
    },
  })
}
...
export function buildTools(): ToolDefinition[] {
  return [
    tool(
      'computer_screenshot',
      'Capture the full screen (or a pixel region) to the session workspace and return the saved PNG path plus dimensions. Pair with vision_glance / vision_ground to locate UI elements.',
      {
      ...
      async (args, exec) => {
        ...
        let rArgs: string[] = []
        if (region.trim() !== '') {
          const parts = region.split(',').map((s: string) => s.trim())
          if (parts.length !== 4 || parts.some((p: string) => !/^\d+$/.test(p))) {
        ...
        if (!m) return { ok: false, error: 'unexpected screenshot output', output: res.stdout.trim().split(/\r?\n/) }
        return { ok: true, path: m[1], width: Number(m[2]), height: Number(m[3]) }
      },
    ),
    tool(
      'computer_cursor',
      'Report the current cursor position as x,y.',
      {},
      async () => {
        const res = await runPs(['-Action', 'cursor'], 10000)
        if (!res.ok) return psOut(res)
        const m = /^cursor:(\d+):(\d+)$/m.exec(res.stdout.trim())
        if (!m) return { ok: false, error: 'unexpected cursor output', output: res.stdout.trim().split(/\r?\n/) }
    ...
    tool(
      'computer_click',
      'Move the cursor to (x, y) and left-click once. Use coordinates from vision_ground / vision_detect on a computer_screenshot.',
      { x: { type: 'integer' }, y: { type: 'integer' } },
      async (args) => {
    ...
    tool(
      'computer_rightclick',
      'Move the cursor to (x, y) and right-click once.',
      { x: { type: 'integer' }, y: { type: 'integer' } },
      async (args) => {
    ...
    tool(
      'computer_drag',
      'Drag the mouse from (fromX, fromY) to (toX, toY). DurationMs controls speed (default 200).',
      {
    ...
    tool(
      'computer_scroll',
      'Scroll the mouse wheel. Positive delta scrolls up, negative scrolls down (e.g. 120 = one notch up, -120 = one notch down).',
      { delta: { type: 'integer', description: 'Wheel delta in multiples of 120.' } },
      async (args) => {
    ...
    tool(
      'computer_key',
      'Send a special key or chord to the focused window, e.g. "{ENTER}", "{TAB}", "{ESC}", "^c", "%{F4}".',
      { key: { type: 'string', required: true, description: 'SendKeys key specification.' } },
      async (args) => {
    ...
    tool(
      'computer_screen',
      'Report the primary screen bounds as x,y,width,height.',
      {},
      async () => {
        const res = await runPs(['-Action', 'screen'], 10000)
        if (!res.ok) return psOut(res)
        const m = /^screen:(-?\d+):(-?\d+):(\d+):(\d+)$/m.exec(res.stdout.trim())
        if (!m) return { ok: false, error: 'unexpected screen output', output: res.stdout.trim().split(/\r?\n/) }
...
export function apply(ctx: any): (() => void) | void {

...
export function buildTools() {
    return [
        ...
        tool('computer_type', 'Type literal text into the focused field. Short plain text (<200 chars) goes through SendKeys; longer or special-character-heavy text is pasted through the clipboard (^v) automatically. SendKeys control sequences like {ENTER}/{TAB}/^c are honored when the text looks like a key sequence.', { text: { type: 'string', required: true, description: 'Text to type, or a SendKeys sequence for key chords.' } }, async (args) => {
